...
def plt_mc_data(ax, X, y, classes,  class_labels=None, map=plt.cm.Paired, 
    ...
    """ Plot multiclass data. Note, if equal_xy is True, setting ylim on the plot may not Course 2 Wk2 Labs """
    for i in range(classes):
        idx = np.where(y == i)
        col = len(idx[0])*[i]
        label = class_labels[i] if class_labels else "c{}".format(i)
        # this didn't Course 2 Wk2 Labs on coursera but did in local version
